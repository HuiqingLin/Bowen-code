...
def time_tran_global( ):
    df= xr.open_dataset("../data/CLM_data/ensmean_I2000Clm50Sp_360x720cru.clm2.h1.0006-01.nc")#以该数据为例制作掩膜
    #mask=mask_time(df)
    ...
    for year in years:
        for month in months:
            df= xr.open_dataset("../data/CLM_data/ensmean_I2000Clm50Sp_360x720cru.clm2.h1.00"+year+"-"+month+".nc")
    ...
    ds.to_netcdf("../data/processed_data/CLM_deal.nc")
...
def convert_to_2d_all():
    df0601= xr.open_dataset("../data/CLM_data/ensmean_I2000Clm50Sp_360x720cru.clm2.h1.0006-01.nc")#读取文件，为了运用其经纬度等信息
    ds=xr.open_dataset("../data/processed_data/CLM_deal.nc")
    ...
    df_HLBowen.to_netcdf("../data/processed_data/CLM_to_2d_120time.nc")

def compare_data_deal( ):
    #inorder to compare FLUXNET data and CLM simulation
    data= pd.read_csv("../data/processed_data/各点多年平均_去2站点.csv")
    df=xr.open_dataset('../data/processed_data/CLM_to_2d_120time.nc')
    ...
    data.to_csv("../data/processed_data/各点多年平均_有经纬LAI_模型.csv")
