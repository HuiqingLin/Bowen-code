...
def make_plot():
    #数据处理
    data= pd.read_csv("../data/processed_data/各点多年平均_有经纬LAI_模型.csv")
    ...
    (c,s)=plot_fitting(data,'b','b_M',ax1,order=1,lw=1.5,color='b',fontsize=10)
    ...
    (c,s)=plot_fitting(data,'H_F_MDS','H_M',ax2, order=1,lw=1.5,color='b',fontsize=10)
    ...
    (c,s)=plot_fitting(data,'LE_F_MDS','LE_M',ax3, order=1,lw=1.5,color='b',fontsize=10)
    ...
    plt.savefig("../data/processed_data/figure/figure 6.png",dpi=600,bbox_inches='tight')
    print("finished!")
