...
def make_plot():
    ...
    y.to_csv("../data/processed_data/各植被Bowen.csv")
    ...
    axes.set_xlabel("Vegetation type",fontsize=12)
    axes.set_ylabel("Bowen ratio",fontsize=12)
    axes.tick_params(labelsize=10)
    x =list(range(len(labels)))
    #plt.axhline(0,linestyle='--', color='k',  linewidth=0.4)
    axes.axhline(0,linestyle='--', color='k',  linewidth=0.4)       
    error_params=dict(elinewidth=0.6,ecolor='black',capsize=1.5,alpha=0.4)
    axes.bar(x, y.b_N, width=width, label='Northern Hemisphere',yerr=y.sem_N,error_kw=error_params,fc='#4984b8',tick_label=labels,edgecolor= 'black',lw=0)
    for a,b in zip(x,y.b_N):
        if b < 0:
            axes.text(a, b,'%.1f'%b, ha = 'center',va = 'top',fontsize=5)
        else:
            axes.text(a, b,'%.1f'%b, ha = 'center',va = 'bottom',fontsize=5)
    
    for i in range(12):
        x[i] = x[i] - width 
    axes.bar(x, y.b, width=width, label='Global',yerr=y.sem_all,error_kw=error_params,fc='#ffb07c',edgecolor= 'black',lw=0)
    ...
    for i in range(12):
        x[i] = x[i] + width *2
    axes.bar(x, y.b_S, width=width, label='Southern Hemisphere',yerr=y.sem_S,error_kw=error_params, fc='mediumturquoise',edgecolor= 'black',lw=0,alpha=0.8)
    ...
    plt.savefig("../data/processed_data/figure/Figure 3.png",dpi=600,bbox_inches='tight')
    print("finished!")
