...
def plot_fitting(df, x_txt, y_txt, ax, order=1,lw=1.5):
    ...
    ax.text(df[x_txt].min(),9,"R="+c+"  "+s,color='b',fontsize=10)
    return c 
...
def make_plot():
    ...
    for t in range(7):
        plot_fitting(df,name_[t],'b',axes[axes_x[t]][axes_y[t]],order=1)        
    #拟合albedo
    df_a=df.dropna()
    plot_fitting(df_a,'albedo','b',axes[1][2],order=1)       
    #纬度分为南北拟合
    df_1=df[df['latitude']>0]
    plot_fitting(df_1,'latitude','b',axes[2][0],order=1)
    df_2=df[df['latitude']<0]
    plot_fitting1(df_2,'latitude','b',axes[2][0],order=1)  
    ...
    plt.colorbar(a,fraction=0.06,pad=0.04,label="Temperature($℃$)")
    plt.savefig("Fluxnet/a_folder/figure/Figure 5.png",dpi=600,bbox_inches='tight')
    print("finished!")
