...
def make_plot():
    ...
    for i in range(3):
        for j in range(3):
            a=axes[i][j].scatter( df[name[i*3+j]],df['H_F_MDS'], c=df['TA_F'],s=10,alpha=1,cmap="RdYlBu_r")
    ...
    #设置纵轴标题
    for z in range(3):
        axes[z][0].set_ylabel("Sensible heat ($W/m^2$)",fontsize=12,labelpad=-1)
    ...
    for t in range(7):
        (c,s)=plot_fitting(df,name_[t],'H_F_MDS',axes[axes_x[t]][axes_y[t]],order=1,lw=1.5,color='k',fontsize=10) 
        axes[axes_x[t]][axes_y[t]].text(df[name_[t]].min(),255,"R="+c+"  "+s,color='k',fontsize=10)  

    #拟合albedo
    df_a=df.dropna()
    (c,s)=plot_fitting(df_a,'albedo','H_F_MDS',axes[1][2],order=1,lw=1.5,color='k',fontsize=10)
    axes[1][2].text(0,255,"R="+c+"  "+s,color='k',fontsize=10)
      
    #纬度分为南北拟合
    df_1=df[df['latitude']>0]
    (c,s)=plot_fitting(df_1,'latitude','H_F_MDS',axes[2][0],order=1,lw=1.5,color='k',fontsize=10)
    axes[2][0].text(2,255,"R="+c+"  "+s,color='k',fontsize=10)

    df_2=df[df['latitude']<0]
    (c,s)=plot_fitting(df_2,'latitude','H_F_MDS',axes[2][0],order=1,lw=1.5,color='g',fontsize=10)
    ...
    plt.savefig("../data/processed_data/figure/Figure S6.png",dpi=600,bbox_inches='tight')
    print("finished!")
