...
def make_plot():
    ...
    for t in range(7):
        plot_fitting(df,name_[t],'H_F_MDS',axes[axes_x[t]][axes_y[t]],order=1)        
    #拟合albedo
    df_a=df.dropna()
    plot_fitting(df_a,'albedo','H_F_MDS',axes[1][2],order=1)       
    #纬度分为南北拟合
    df_1=df[df['latitude']>0]
    plot_fitting(df_1,'latitude','H_F_MDS',axes[2][0],order=1)
    df_2=df[df['latitude']<0]
    plot_fitting1(df_2,'latitude','H_F_MDS',axes[2][0],order=1)  
    ...
    plt.savefig("Fluxnet/a_folder/figure/Figure S6.png",dpi=600,bbox_inches='tight')
    print("finished!")
