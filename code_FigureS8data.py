...
def Deal_FigS8_data():
    df_h1= xr.open_dataset("../data/CLM_data/ensmean_0615h1.nc")
    df_h1=df_h1.mean(dim='time')
    df_b1= xr.open_dataset("../data/Fluxnet/a_folder/CLM_deal.nc") #为了读取H和LE
    ...
    var_h1_out.to_csv("../data/processed_data/FigS8_var_h1_out.csv")
    
    #接下来处理h0和影响因子
    #为了提取每个网格主导的pft，数据格式有误，先把数据重新封装
    df_pft= xr.open_dataset("../data/CLM_data_h0/surfdata_360x720cru_simyr2000_c130418_pft_land.nc")
    df_pft1=xr.Dataset(
               {
    ...
    df_pftmax.to_netcdf("../data/Fluxnet/a_folder/CLM_pftmax.nc")

    #提取H和LE
    #因为之前处理的/CLM_to_2d_120time.nc文件太大，所以用cdo处理为多年平均，此操作在ubuntu terminal进行
    #cdo timmean CLM_to_2d_120time.nc CLM_to_2d.nc

    df_b0= xr.open_dataset("../data/processed_data/CLM_to_2d.nc") #为了读取H和LE
    lat=df_b0.lat
    lon=df_b0.lon
    df_HLE=pd.DataFrame( )
    x=0
    ...
    #处理h0数据
    df_h0= xr.open_dataset("../data/CLM_data_h0/ensmean_0615h0.nc")
    ...
    var_h0_out.to_csv("../data/processed_data/FigS8_var_h0_out.csv")
    print("finished!")
