...
def Deal(fod):
    ...
    df_12.to_csv("Fluxnet/all/"+fod+"_all.csv",encoding='utf-8') 
...
def Deal_vegetation():
    ...
    fod=["CRO","CSH","DBF","DBF_S","DNF","EBF","EBF_S","ENF","ENF_S","GRA","GRA_S","MF","MF_S","OSH", "SAV", "SAV_S","WET","WET_S","WSA","WSA_S"]   
    for i in range(len(fod)):
        (df_site,df_12)=Deal(fod[i])
        df_205=pd.concat([df_205,df_site])
        df_205_12=pd.concat([df_205_12,df_12])
    
    #去除有效值小于35%的行并保存
    df_drop=df_205_12[df_205_12['valid']>0.35]
    df_drop.to_csv("Fluxnet/a_folder/多点12个月份平均_去2站点.csv")
    df_mean=df_drop.groupby(["type","month"]).mean()
    df_mean.eval('b=H_F_MDS/LE_F_MDS',inplace=True) 
    df_mean.to_csv("Fluxnet/a_folder/各植被12个月份平均.csv")
    ...
    df_205_.insert(len(df_205_.columns),'IGBP',df_info.IGBP)
    ...
    df_205drop.to_csv("Fluxnet/a_folder/各点多年平均_去2站点.csv")
